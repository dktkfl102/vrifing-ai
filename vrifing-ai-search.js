...
const BASE_URL = "https://node.apiopenperplex.com/search";
...
app.get("/fetch-data", async (req, res) => {
  try {
    ...
    If the information is incomplete or unavailable on Naver, then search on Google Maps as a secondary option. 
    Provide the results in the following JSON format: { 
        ...
        "store_location": { 
            "latitude": "Latitude from Google",
            "longitude": "Longitude from Google"
        }, 
        "categories": [ 
            { 
    ...
    For store_location, latitude and longitude, infer the values based on the provided address.
    If some information cannot be found, leave it empty aside from store_location.
    If no information is found at all, simply respond with { "error": "Information not available" }.
    
    region name: ${region}, store name: ${store} 딱 obj만 내려줘 앞뒤 설명 넣지 말고`;

    const userPrompt = `region name: ${region}, store name: ${store} 딱 obj만 내려줘 앞뒤 설명 넣지 말고`;

    const response = await axios.get(`${BASE_URL}`, {
      params: { query: systemPrompt, location: "ko" }, // params 객체로 전달
